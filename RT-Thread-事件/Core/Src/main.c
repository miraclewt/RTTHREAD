...
static void receive_thread_entry(void *parametr)
{
  ...
  while (1)
  {
    ...
    rt_event_recv(test_event,
                  ...
                  RT_WAITING_FOREVER,
                  &receved);
    if (receved == (KEY1_EVENT | KEY2_EVENT))
    {
      /* code */
      rt_kprintf("KEY1,KEY2按下，获取时间成功\r\n");
...
static void send_thread_entry(void *parameter)
{
  ...
  {
    /* code */
    if (HAL_GPIO_ReadPin(Key1_GPIO_Port, Key1_Pin) == RT_TRUE)
    {
      /* code */
      rt_kprintf("KEY1 SET DOWN\r\n");
      rt_event_send(test_event, KEY1_EVENT);
    }
    if (HAL_GPIO_ReadPin(Key2_GPIO_Port, Key2_Pin) == RT_TRUE)
    {
      /* code */
      rt_kprintf("KEY2 SET DOWN\r\n");
